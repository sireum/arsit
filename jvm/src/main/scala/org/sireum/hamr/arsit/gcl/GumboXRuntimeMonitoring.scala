  ...
  def handlePlatformProviderPlugin(rmContainer: RM_Container,
    ...
    val runtimeMonitorExt: ST =
      st"""package $runtimePackage
          ...
          |
          |    ${if (!hasGcl) "// " else ""}new GumboXRuntimeMonitor(Map.empty ++ (for(e <- HamrVision.threadNickNames.entries) yield (e._2, e._1))),
          |
    ...
    val gui: ST =
      st"""package ${runtimePackage}
          ...
          |
          |class GumboXRuntimeMonitor(threadNickNames: Map[BridgeId, String]) extends JFrame with RuntimeMonitorListener {
          |
