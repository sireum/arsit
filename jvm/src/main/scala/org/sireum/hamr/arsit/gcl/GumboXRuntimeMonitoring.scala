...
object GumboXRuntimeMonitoring {
  ...
  def handlePlatformProviderPlugin(rmContainer: RM_Container,
    ...
    val runtimeMonitorExt: ST =
      st"""package $runtimePackage
          ...
          |
          |    ${if (!hasGcl) "// " else ""}new DefaultRuntimeMonitor(Map.empty ++ (for(e <- HamrVision.threadNickNames.entries) yield (e._2, e._1))),
          |
          |    HamrVision.getRuntimeMonitor()
          |
          |    ${drmMarker.endMarker}
          |  )
          |
    ...
    val gui: ST =
      st"""package ${runtimePackage}
          ...
          |
          |class DefaultRuntimeMonitor(threadNickNames: Map[BridgeId, String]) extends JFrame with RuntimeMonitorListener {
          |
          |  val testDir = Os.path(".") / "src" / "test" / "bridge" / "isolette"
          |
          |  var jtable: JTable = _
          |  var model: TableModel = _
          |  var scrollToBotom: B = T
          |
          |  def init(modelInfo: ModelInfo): Unit = {
          |    this.setTitle("Visualizer")
          |
          |    model = new TableModel(threadNickNames)
          |    jtable = new JTable()
          ...
          |
          |    val chkScroll = new JCheckBox("Scroll to botoom")
          |    chkScroll.setSelected(scrollToBotom)
          |    chkScroll.addActionListener(e =>
          |      scrollToBotom = chkScroll.isSelected
          |    )
  ...
  def getHamrVision(basePackage: String): ST = {
    val ret = st"""// #Sireum
                  ...
                  |
                  |import org.sireum._
                  |import art.scheduling.static.{CommandProvider, InfoThreadNickNames}
                  |
                  |// Do not edit this file as it will be overwritten if HAMR codegen is rerun
                  |
                  |object HamrVision {
                  |  // modify this to specify the thread order in the gui table.
                  |  // If its empty then a default ordering will be used, otherwise
                  |  // it must be the case that each BridgeId appears once in the sequence
                  |  var displayOrder: ISZ[art.Art.BridgeId] = IS()
                  |  var threadNickNames: Map[String, art.Art.BridgeId] = Map.empty
                  |
                  |  var cp: Option[CommandProvider] = None()
                  |  var vision: Option[RuntimeMonitorListener] = None()
                  |
                  |  def getCommandProvider(numSlots: Z): CommandProvider = {
                  |    cp = Some(HamrVisionRuntimeMonitorI.getCommandProvider(numSlots, threadNickNames))
                  |    return cp.get
                  |  }
                  |
                  |  def getRuntimeMonitor(): RuntimeMonitorListener = {
                  |    vision = Some(HamrVisionRuntimeMonitorI.getVision(displayOrder, threadNickNames, cp))
                  |    return vision.get
                  |  }
                  |}
                  |
                  |@ext object HamrVisionRuntimeMonitorI {
                  |  def getCommandProvider(numSlots: Z,
                  |                         threadNickNames: Map[String, art.Art.BridgeId]): CommandProvider = $$
                  |
                  |  def getVision(displayOrder: ISZ[art.Art.BridgeId],
                  |                threadNickNames: Map[String, art.Art.BridgeId],
                  |                cp: Option[CommandProvider]): RuntimeMonitorListener = $$
                  |}"""
    return ret
  }
  ...
  def getHamrVision_Ext(basePackage: String): ST = {
    val ret =
      st"""package ${basePackage}.runtimemonitor
          ...
          |  def getCommandProvider(numSlots: Z,
          |                         threadNickNames: Map[String, art.Art.BridgeId]): CommandProvider = {
          |    return new HamrVisionCommandProvider(numSlots, threadNickNames)
          |  }
          |
          |  def getVision(displayOrder: ISZ[art.Art.BridgeId],
          |                threadNickNames: Map[String, art.Art.BridgeId],
          |                cp: Option[CommandProvider]): RuntimeMonitorListener = {
          |    return new HamrVisionRuntimeMonitor(displayOrder, threadNickNames, cp)
          |  }
          |}
          |
          |class HamrVisionCommandProvider(numSlots: Z,
          |                                threadNickNames: Map[String, art.Art.BridgeId]) extends JPanel with CommandProvider {
          |
          ...
          |
          |    val cmbThreads = new JComboBox[Predef.String](new java.util.Vector((for (t <- threadNickNames.keys) yield t.native).elements.asJava))
          ...
          |}
          |
          |class HamrVisionRuntimeMonitor(displayOrder: ISZ[art.Art.BridgeId],
          |                               threadNickNames: Map[String, art.Art.BridgeId],
          |                               cp: Option[CommandProvider]) extends RuntimeMonitorListener {
          |
          |  var visionTreeTable: JTreeTable = _
          |
          |  def clearColors(): Unit = {
          |    visionTreeTable.hyperPeriod()
          |  }
          |
          |  def modelInfoEntry(modelInfo: ModelInfo): ISZ[Entry] = {
          |    val revMap = Map.empty ++ (for(e <- threadNickNames.entries) yield (e._2, e._1))
          |
          |    val orderComponents: ISZ[Component] =
          |      if (displayOrder.nonEmpty) {
          |        val map = Map.empty ++ (for (c <- modelInfo.components) yield (art.Art.BridgeId.fromZ(c.id), c))
          |        val uniqueIds = Set.empty[art.Art.BridgeId]() ++ displayOrder
          |        for(id <- uniqueIds.elements) yield map.get(id).get
          |      }
          |      else modelInfo.components
          |
          ...
          |    cp match {
          |      case Some(i: HamrVisionCommandProvider) =>
          |        visionFrame.add(i, BorderLayout.SOUTH)
          |        i.clearColorsCallback = clearColors _
          |      case Some(x) => halt(s"Not expecting $$x")
          |      case _ =>
          |    }
          |    visionFrame.pack()
