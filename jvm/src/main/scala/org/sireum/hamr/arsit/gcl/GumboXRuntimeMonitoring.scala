...
object GumboXRuntimeMonitoring {
  ...
  def handlePlatformProviderPlugin(rmContainer: RM_Container,
    ...
    val runtimeMonitorExt: ST =
      st"""package $runtimePackage
          ...
          |
          |    ${if (!hasGcl) "// " else ""}new DefaultRuntimeMonitor(),
          |
  ...
  def getHamrVision(basePackage: String): ST = {
    val ret = st"""// #Sireum
                  ...
                  |    vision = Some(HamrVisionRuntimeMonitorI.getVision(cp))
                  |    return vision.get
                  |  }
                  |}
                  |
                  ...
                  |                         threadNickNames: ISZ[String]): CommandProvider = $$
                  |
                  |  def getVision(cp: Option[CommandProvider]): RuntimeMonitorListener = $$
                  |}"""
    return ret
  }
  ...
  def getHamrVision_Ext(basePackage: String): ST = {
    val ret =
      st"""package ${basePackage}.runtimemonitor
          ...
          |
          |  def getVision(cp: Option[CommandProvider]): RuntimeMonitorListener = {
          |    return new HamrVisionRuntimeMonitor(cp)
          |  }
          |}
          |
          |class HamrVisionCommandProvider(numSlots: Z,
          |                                threadNickNames: ISZ[String]) extends JPanel with CommandProvider {
          |
          ...
          |
          |    val cmbThreads = new JComboBox[Predef.String](new java.util.Vector((for (t <- threadNickNames) yield t.native).elements.asJava))
          ...
          |}
          |
          |class HamrVisionRuntimeMonitor(cp: Option[CommandProvider]) extends RuntimeMonitorListener {
          |
          ...
          |
          |    val micomps: ISZ[Component] =
          |      if (HamrVision.optDisplayOrder.nonEmpty) {
          |        val map = Map.empty ++ (for (c <- modelInfo.components) yield (art.Art.BridgeId.fromZ(c.id), c))
          |        val uniqueIds = Set.empty[art.Art.BridgeId]() ++ HamrVision.optDisplayOrder
