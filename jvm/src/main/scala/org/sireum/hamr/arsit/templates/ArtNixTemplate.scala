...
object ArtNixTemplate {
  ...
  @pure def aep(packageName: String,
    ...
    @pure def app(packageName: String,
      ...
      def localPortId(p: Port): String = { return s"${p.name}PortId" }

      def localPortIdOpt(p: Port): String = { return s"${localPortId(p)}Opt" }
      
      val dispatchTriggers: Option[ISZ[String]] = SlangUtil.getDispatchTriggers(component)
      val inPorts: ISZ[Port] = SlangUtil.getFeatureEnds(component.features).filter((p: ir.FeatureEnd) => CommonUtil.isInPort(p)).map((f: ir.FeatureEnd) => {
        ...
        SlangUtil.getPort(f, component, types, basePackage, isTrigger, z"-1000")
      })
      ...
      val body: ST = {
          ...
          val loopBody: ST = {
            ...
            val receiveOnInPorts: ISZ[ST] =
              inPorts.map((p: Port) => {
                val dispatch: String = if(CommonUtil.isAadlDataPort(p.feature)) { "F" } else { "T" }
                st"""{
    ...
    @pure def PlatformNix(packageName: String): ST = {
      ...
      val init: ST = st"""val id = seed + port
                         |SharedMemory.create(id)
                         |ids = ids :+ id"""

      val receive: ST = st"""portOpt match {
                            |  case Some(port) =>
                            |    out.value1 = port
      ...
      val finalise: ST = st"""for (id <- ids) {
                             |  SharedMemory.remove(id)
                             |}"""

      val receiveAsync: ST = st"""portOpt match {
  ...
  @pure def transpilerX(opts: CTranspilerOption,
    ...
    val script_home = s"$${${SlangUtil.SCRIPT_HOME}}"

    val projHomesRel = opts.sourcepath.map((s: String) => SlangUtil.relativizePaths(dirs.binDir, s, script_home))
    val cOutputDirRel = SlangUtil.relativizePaths(dirs.binDir, opts.output.get, script_home)

    val path_sep = s"$${PATH_SEP}"

    val ret = st"""#!/usr/bin/env bash
                  |#
                  |# This file is autogenerated.  Do not edit
                  |#
                  |set -e
                  ...
                  |fi
                  |
                  |${SlangUtil.SCRIPT_HOME}=$$( cd "$$( dirname "$$0" )" &> /dev/null && pwd )
    ...
    if(opts.exts.nonEmpty) {
      val extsRel = opts.exts.map((s: String) => SlangUtil.relativizePaths(dirs.binDir, s, script_home))
      extras = extras :+ st""" \
